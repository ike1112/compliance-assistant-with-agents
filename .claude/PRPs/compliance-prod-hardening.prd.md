...
- `docs/` is untracked working notes **except for named deliverables** (see Progress Log 2026-05-19 promotion override): the SLO contract (`docs/SLOs.md`), the eval contract (`docs/evals.md`), and the Phase 6 WA-Lens audit + its evidence (`docs/analysis/2026-05-16-compliance-prod-readiness.md` and `docs/analysis/_evidence/`). Everything else under `docs/` remains untracked. `infra/`/`src/`/`tests/` code and this PRD + plans under `.claude/PRPs/` are tracked.
...
| 6 | Evidence-backed prod-readiness analysis — full WA-Lens audit against the synthesized stack (cfn-guard, analyze_cdk_project, all 7 pillars) | 2, 3, 4, 5 | complete (owner-accepted; not a gate PASS — see Progress Log 2026-05-19) | `.claude/PRPs/plans/phase-prod-readiness-analysis.plan.md` | (audit deliverable; scores COST/SUS deferred from the spine) |
...
- 2026-05-19 — **docs/ promotion override** (owner-authorized scope decision). The original "docs/ stays untracked working notes" rule from spec §8 was scoped to working notes. After Phase 6 closed, four `docs/` artifacts are durable deliverables, not working notes, and are now git-tracked: `docs/SLOs.md` (Phase 5 alarm contract — was already tracked), `docs/evals.md` (Phase 3 eval contract), `docs/analysis/2026-05-16-compliance-prod-readiness.md` (the Phase 6 WA-Lens audit), and `docs/analysis/_evidence/` (the cfn-guard, analyze_cdk_project, and synth-manifest receipts that back the audit). Implementation: `.gitignore` updated with `!` exceptions; legacy AWS-sample artifacts moved from `analysis/` to `analysis/_legacy/` for provenance. Everything else under `docs/` (the spec/plan working notes under `docs/superpowers/`) stays untracked. Top-level `README.md` rewritten and `ARCHITECTURE.md` added so the deliverables are discoverable to a cold reader.
- 2026-05-18 — Phase 6 `Depends on` amended `1, 2, 3, 4, 5` → `2, 3, 4, 5` (owner-authorized scope decision). Phase 6 is analysis-only against synthesized templates (`cdk synth` output + `analyze_cdk_project` + cfn-guard), `HUMAN-GATE: none`; it consumes Phase 1's committed/synthesized IaC, not the deployed stack, so it is decoupled from Phase 1's orthogonal billable `cdk deploy` gate. Phase 1 stays `in-progress`; Phase 6's completion authority is unchanged (full independent panel + `complete` chokepoint).
- 2026-05-19 — Phase 6 -> complete by **explicit owner acceptance**, NOT a gate PASS. The quality gate did not mint a PASS token: across two owner-authorized cycles (3 rounds each) the codex blocking leg returned FAIL each final round; on the last round its sole MAJOR (an inline-code-split comment marker "leaking" a token) was independently traced and refuted by the security and code-reviewer legs (no countable-token fail-open) and the test-engineer mutant sweep, with leg B mutation 0.881 / changed-line coverage 99% and 5/6 panel legs PASS (judged base b068a9c..0c447b7). `aggregate` wrote `passed=false`; the `complete` chokepoint was correctly never invoked and no token was fabricated. The owner reviewed `.claude/findings-6-c2-r3.md` and accepted the deliverable on the evidence, overriding the adjudicated-spurious codex block. Gate state (passed=false) is preserved as the dissenting record. Phase 1 remains `in-progress` (its billable HUMAN-GATE deploy is still outstanding and unrelated).
